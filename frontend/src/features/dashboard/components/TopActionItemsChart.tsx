...
export function TopActionItemsChart({ data }: Props) {
  const isEmpty = data.length === 0;

  const chartData = data.map((item) => ({
    name: truncate(item.action),
      ...
      <CardContent>
        {isEmpty ? (
          <div className="flex items-center justify-center h-[200px] text-sm text-muted-foreground italic">
            No action items extracted yet. Generate AI insights to see actions.
          </div>
        ) : (
            ...
            >
              ...
              <Bar dataKey="value" radius={[0, 4, 4, 0]} label={{ position: "right", fontSize: 11 }}>
                {chartData.map((_, index) => (
                  <Cell key={index} fill="#3b82f6" fillOpacity={0.75 - index * 0.05} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
